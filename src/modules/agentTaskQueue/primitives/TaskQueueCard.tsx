...
import { Play, Pause, ChevronUp, ChevronDown, Trash2, Cpu } from 'lucide-react';
...
export function TaskQueueCard({ task, index, totalTasks, onReorder, onTogglePause, onCancel }: TaskQueueCardProps) {
  const isRunning = task.status === 'running';
  const isPaused = task.status === 'paused';

  return (
    <div className={`p-3 rounded-xl border transition-all flex flex-col gap-2 ${
      isRunning ? 'bg-indigo-50/60 border-indigo-200' : isPaused ? 'bg-zinc-100/70 border-zinc-200 opacity-70' : 'bg-zinc-50 border-zinc-200/90'
    }`}>
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded capitalize ${
            isRunning ? 'bg-indigo-100 text-indigo-800' : isPaused ? 'bg-amber-100 text-amber-800' : 'bg-zinc-200 text-zinc-700'
          }`}>{task.status}</span>
        </div>
      </div>
      ...
      {(isRunning || task.progress > 0) && (
        <div className="flex flex-col gap-1">
          ...
          <div className="w-full bg-zinc-200 h-1 rounded-full overflow-hidden">
            <div className={`h-full transition-all duration-300 ${isPaused ? 'bg-amber-500' : 'bg-indigo-600'}`} style={{ width: `${task.progress}%` }} />
          </div>
        </div>
      )}
