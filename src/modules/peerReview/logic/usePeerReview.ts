...
export function usePeerReview(repoFullName: string) {
  ...
  const reviewPullRequest = async (prNumber: number) => {
    ...
    try {
      ...
      if (token) {
        try {
          const findingsList = rep.findings.map((f: any) => `- [${f.severity.toUpperCase()}] **${f.file || 'General'}**: ${f.message}`).join('\n');
          const commentBody = `### 🤖 AI Peer Review Report\n\n**Status:** ${rep.overallStatus.toUpperCase()}\n**Skor:** ${rep.score}/100\n\n**Ringkasan:**\n${rep.summary}\n\n**Temuan Utama:**\n${findingsList}\n\n_Auto-generated by AI Studio Dev Copilot_`;
          await peerReviewApi.postPrComment(repoFullName, prNumber, commentBody, token);
        } catch (e) {
